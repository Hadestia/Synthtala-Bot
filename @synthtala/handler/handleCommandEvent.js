					...
					if (event.type === definedType) {
						...
						try {
							if (moduleScript.handleEvent && typeof(moduleScript.handleEvent) === 'function') {
								Logger.makeLog(GLOBAL.CLIENT.LOG_PATH, `Command-Event ${cmd_id} was called at thread-${threadID}.`, 'module');
							...
							Logger.makeLog(GLOBAL.CLIENT.LOG_PATH, `Command-Event ${cmd_id} ERROR: ${err}`, 'error');
							console.error(err);
						}
					}
				}
