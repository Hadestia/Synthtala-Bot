...
const CLIENT = {
	...
	SERVER_LINK: `https://nasmerah-bot.onrender.com`,
...
const start_server = async function () {
	...
	await App.listen(App.get('port'), async () => {
		...
		Axios.get(encodeURI(`${CLIENT.SIDE_SERVER_LINK}/autoPing?name=Nasmerah-Bot-Get&link=${CLIENT.SERVER_LINK}`)).then(()=> {}).catch(()=>{});
	});
}
...
const projectInfo = Filesystem.readJsonSync(Path.join(__dirname, 'package.json'));
		...
		child.on('message', async function ( data ) {
			...
			switch (msg_signal) {
				...
				case '-logged': {
					...
					const restart_count = (CLIENT.ACTIVE_BOT[data.id]) ? CLIENT.ACTIVE_BOT[data.id].restart_count + 1 : 0;
					CLIENT.ACTIVE_BOT[data.id] = {
						id: data.id,
						name: data.name,
