...
const reference = require('../json/ref-fonts.json');
...
module.exports.get = async function (text, style) {
	...
	if (styleNum == 1 || style == 'bold-sans') {
		return replaceEachChar(
			text,
			reference.bold_sans_caps,
			reference.bold_sans_small,
			reference.bold_sans_number
		);
	} else if (styleNum == 2 || style == 'bold-sans-italic') {
		return replaceEachChar(
			text,
			reference.bold_sans_italic_caps,
			reference.bold_sans_italic_small,
			reference.bold_sans_number
		);
	} else if (styleNum == 3 || style == 'bold-serif') {
		return replaceEachChar(
			text,
			reference.bold_serif_caps,
			reference.bold_serif_small,
			reference.bold_serif_number
		);
	} else if (styleNum == 4 || style == 'bold-serif-italic') {
		return replaceEachChar(
			text,
			reference.bold_serif_italic_caps,
			reference.bold_serif_italic_small,
			reference.bold_serif_number
		);
	} else if (styleNum == 5 || style == 'bold-medieval') {
		return replaceEachChar(
			text,
			reference.bold_medieval_caps,
			reference.bold_medieval_small,
			reference.bold_medieval_number
		);
	} else if (styleNum == 6 || style == 'thin-font1') {
		return replaceEachChar(
			text,
			reference.thin_font_1_caps,
			reference.thin_font_1_small,
			reference.thin_font_1_number
		);
	} else {
